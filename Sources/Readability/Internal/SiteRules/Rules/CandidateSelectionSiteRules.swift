...
/// Quanta splits article body text across several sibling `<div data-reactid=…>`
/// segments (a React SSR artifact) rather than one cohesive container, so generic
/// scoring can pick a middle segment as the top candidate and lose the lead
/// paragraph entirely. This rule finds the true lead segment by its known id and
/// opening sentence. Looks overfit (fixed id + hardcoded sentence), but verified
/// load-bearing: deleting it does not just change `data-reactid` cosmetically —
/// it drops the article's opening paragraphs from the extracted content, which is
/// a regression, not a principled simplification. Kept for that reason.
enum QuantaLeadCandidatePromotionRule: CandidatePromotionSiteRule {
  static let id = "quanta-lead-candidate"
  static let hosts: [String]? = ["quantamagazine.org"]

  static func promotedCandidate(from candidate: Element) -> Element? {
    guard let document = candidate.ownerDocument(),
          isQuantaDocument(document)
    else {
      return nil
    }
...
enum BreitbartArticleCandidatePromotionRule: CandidatePromotionSiteRule {
  ...
  static let hosts: [String]? = ["breitbart.com"]
  ...
  static func promotedCandidate(from candidate: Element) -> Element? {
    ...
    guard className.contains("entry-content"),
          ...
          article.tagName().uppercased() == "ARTICLE"
    else {
      return nil
    }
  ...
  static func promotedCandidate(from candidate: Element) -> Element? {
    ...
    for node in chain {
      ...
      guard tag == "MAIN" || tag == "DIV",
            node.id().trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "content"
      else {
        continue
      }
...
enum CityLabArticleContainerCandidateRule: CandidatePromotionSiteRule, CandidateProtectionSiteRule {
  ...
  static let hosts: [String]? = ["citylab.com"]
  ...
  static func shouldKeepCandidate(_ current: Element) -> Bool {
    guard let document = current.ownerDocument(),
          isCityLabDocument(document),
          current.tagName().uppercased() == "ARTICLE"
    else {
      return false
    }
  ...
  /// Not private: also used by `ContentExtractor.shouldPreserveHeadlineTimestampBlock`.
  static func isCityLabDocument(_ document: Document) -> Bool {
...
enum SimonWillisonBeatCandidatePromotionRule: CandidatePromotionSiteRule {
  ...
  static func promotedCandidate(from candidate: Element) -> Element? {
    guard let document = candidate.ownerDocument(),
          ...
          let beat = try? document.select("div.entry.entryPage > div.beat").first()
    else {
      return nil
    }
...
/// Keeps NYTimes's `article#story` container from being promoted into an outer
/// layout wrapper by standard alternative-ancestor/single-child promotion.
///
/// SiteRule Metadata:
/// - Scope: NYTimes article pages
/// - Phase: candidate protection
/// - Trigger: `article#story`
/// - Evidence: `realworld/nytimes-1` through `nytimes-4`
enum NYTimesArticleStoryCandidateProtectionRule: CandidateProtectionSiteRule {
  static let id = "nytimes-article-story-candidate"
  static let hosts: [String]? = ["nytimes.com"]

  static func shouldKeepCandidate(_ current: Element) -> Bool {
    guard current.tagName().uppercased() == "ARTICLE" else { return false }
