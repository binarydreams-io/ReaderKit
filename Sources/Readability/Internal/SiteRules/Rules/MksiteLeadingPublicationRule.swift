...
enum MksiteLeadingPublicationRule: ArticleCleanerSiteRule {
  ...
  static func apply(to articleContent: Element, context _: ArticleCleanerSiteRuleContext) throws {
    ...
    guard let publicationIndex = leadingPublicationNodeIndex(in: childNodes),
          ...
          try isLeadingPublicationElement(publication)
    else {
      return
    }
    ...
    guard sawTagLink,
          ...
          try isLeadMediaElement(nextElement)
    else {
  ...
  private static func matchesMksiteContextIfAvailable(
  ...
  private static func isLeadMediaElement(_ element: Element) throws -> Bool {
    ...
    if tagName == "center" {
      return (try? element.select("a, img").isEmpty()) == false
    }
