...
enum CityLabHeadlineTimestampRule: SerializationSiteRule {
  ...
  static func apply(to articleContent: Element) throws {
    ...
    for wrapper in try articleContent.select("p").reversed() {
      ...
      guard children.count == 1,
            ...
            headline.tagName().lowercased() == "h2"
      else {
        continue
      }
  ...
  private static func formatCityLabTime(_ iso8601: String) -> String? {
    ...
    guard let regex = try? NSRegularExpression(pattern: pattern),
          ...
          let hour = Int(iso8601[hourRange])
    else {
      return nil
    }
...
/// Keeps CityLab's compact headline+timestamp header from being removed as a
/// duplicate of the article title — CityLab's `h2[itemprop=headline]` template
/// uses the `l-article__hed` class.
///
/// SiteRule Metadata:
/// - Scope: CityLab article header
/// - Phase: candidate collection (headline duplicate-title stripping)
/// - Trigger: `.l-article__hed` class, or any CityLab document
/// - Evidence: `realworld/citylab-1`
enum CityLabHeadlineTimestampRetentionRule: HeadlineTimestampRetentionSiteRule {
  static let id = "citylab-headline-timestamp-retention"
  static let hosts: [String]? = ["citylab.com"]
  ...
  static func shouldPreserveHeadlineTimestampBlock(_ header: Element) -> Bool {
    ...
    if let doc = header.ownerDocument(), CityLabArticleContainerCandidateRule.isCityLabDocument(doc) {
      return true
    }
    return false
  }
