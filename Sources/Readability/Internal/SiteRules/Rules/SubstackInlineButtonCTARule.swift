...
enum SubstackInlineButtonCTARule: ArticleCleanerSiteRule {
  ...
  static let hosts: [String]? = ["substack.com"]
  ...
  private static func isShareButton(_ element: Element) -> Bool {
    ...
    if hasSharePayload(element) {
      return true
    }
    if hasShareLink(element) {
      return true
    }
  ...
  private static func hasSharePayload(_ element: Element) -> Bool {
    guard let raw = try? element.attr("data-attrs"),
          ...
          let payload = try? JSONDecoder().decode(ButtonPayload.self, from: data)
    else {
      return false
    }
  ...
  private static func hasSubstackContext(
    ...
    if hasSubstackArticleElement(articleContent) {
      return true
    }
    if isInsideBodyMarkup(element) {
      return true
    }
    return false
  }
