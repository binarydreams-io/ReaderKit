...
final class NodeCleaner {
  ...
  private let shouldKeepUnlikelyCandidateHook: (Element) -> Bool
  ...
  init(
    ...
    shouldKeepUnlikelyCandidate: @escaping (Element) -> Bool = { _ in false }
  ) {
    self.options = options
    self.shouldKeepBylineContainerHook = shouldKeepBylineContainer
    self.shouldKeepUnlikelyCandidateHook = shouldKeepUnlikelyCandidate
  }
  ...
  private func shouldRemoveAsUnlikelyCandidate(_ element: Element, matchString: String) -> Bool {
    ...
    // Site rules can protect structurally-legitimate content that collides
    // with a generic unlikely-candidate pattern (e.g. Firefox Nightly's
    // "#comments"/"#adjacent-posts" ids, The Verge's "newsletter-wrapper" class).
    if shouldKeepUnlikelyCandidateHook(element) {
      return false
    }
  ...
  func isProbablyVisible(_ node: Element) -> Bool {
    VisibilityRules.isProbablyVisibleForScoring(node)
  }
