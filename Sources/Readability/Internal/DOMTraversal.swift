...
enum DOMTraversal {
  private static let phrasingTags = Set(Configuration.phrasingElements.map { $0.uppercased() })
  private static let phrasingContainerTags: Set<String> = ["A", "DEL", "INS"]

  /// Check if node is phrasing content (inline content).
  /// Canonical implementation shared by `ArticleCleaner`, `ContentExtractor`,
  /// and `Readability.prepDocument` — the tag sets are computed once here
  /// rather than being rebuilt on every call.
  static func isPhrasingContent(_ node: Node) -> Bool {
    ...
    if phrasingContainerTags.contains(tagName) {
...
extension Element {
  ...
  func hasAncestor(tagName: String, maxDepth: Int = 3, filter: ((Element) -> Bool)? = nil) -> Bool {
    DOMTraversal.hasAncestorTag(self, tagName: tagName, maxDepth: maxDepth, filter: filter)
  }
}
